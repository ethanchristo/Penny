...
struct InsightsView: View {
    ...
    var body: some View {
        ...
        .navigationDestination(for: InsightsOptions.self) { option in
            Group {
                switch option {
        ...
        .navigationDestination(for: InsightsRoute.self) { route in
            ...
            case .fund(let id):
                if let fund = funds.first(where: { $0.persistentModelID == id }) {
    ...
    private var headerMenu: some View {
        Menu {
            ...
            if !addableFunds.isEmpty {
                Menu("Add Fund", systemImage: "rectangle.stack") {
                    ForEach(addableFunds) { fund in
                        Button("\(fund.symbol)  \(fund.name)") {
                            add(.fund(fund.persistentModelID))
                        }
                    }
                }
            }
    ...
    private func navigableTile(for tile: InsightTile) -> some View {
        ...
        case .categoryBudget(let id):
            if let category = categories.first(where: { $0.persistentModelID == id }) {
                ...
                NavigationLink(value: InsightsRoute.categoryBudget(id)) {
        ...
        case .overallBudget:
            NavigationLink(value: InsightsRoute.overallBudget) {
        ...
        case .fund(let id):
            if let fund = funds.first(where: { $0.persistentModelID == id }) {
                NavigationLink(value: InsightsRoute.fund(id)) {
                    FundCell(fund: fund)
                }
                .buttonStyle(.plain)
    ...
    private func reorderLabel(for tile: InsightTile) -> Text {
        ...
        case .fund(let id):
            let fund = funds.first { $0.persistentModelID == id }
    ...
    private func staticTile(for tile: InsightTile) -> some View {
        ...
        case .fund(let id):
            if let fund = funds.first(where: { $0.persistentModelID == id }) {
                FundCell(fund: fund, interactive: false)
            }
        }
    }
...
//        let tint: Color = isBuiltin ? (tile.isHidden ? Color(.systemGreen) : .clear) : Color(.systemRed)
    ...
    private func isResolvable(_ tile: InsightTile) -> Bool {
        ...
        case .fund(let id):
            return funds.contains { $0.persistentModelID == id }
        }
    }
    ...
    private var addableFunds: [Fund] {
        funds.filter { fund in
            !tiles.contains { $0.kind == .fund(fund.persistentModelID) }
        }
    }
...
//        }
    ...
    private func average() -> Double {
        let total = netTotalType(for: transactions, funds: [], in: timeRange, offset: 0, type: .timeRange)
